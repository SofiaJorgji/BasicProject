...
public class TC10 extends AdactinTest {

	@Test
	public void ap10() {
		new Select(getDriver().findElement(By.id("location")))
				.selectByVisibleText("Melbourne");
		...
		String gst = getDriver().findElement(By.id("gst_dis")).getAttribute("value");
		String[] splittingGST = gst.split(" ");
		String lastEl = splittingGST[splittingGST.length - 1];
		double GSTCalculation = Double.parseDouble(lastEl);
		// Calculate total price excluding GST
		double expectedTotalPriceWithoutGST =
				(double) pricePerNight * numberOfRooms * numberOfNights;
		double expectedTotalPrice = expectedTotalPriceWithoutGST + GSTCalculation;
